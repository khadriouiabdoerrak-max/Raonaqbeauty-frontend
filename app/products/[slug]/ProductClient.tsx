...
const BUY_TRUST = [
  { t: "Livraison rapide", d: "au Maroc" },
  { t: "Paiement à la livraison", d: "inspectez d’abord" },
  { t: "Pièce Raonaq", d: "écrin d’origine" },
...
function BuyPanel({
  ...
  return (
    <div>
      ...
      <h1 className="font-display mt-1 text-4xl font-semibold leading-none tracking-wide text-[#1C1412] md:text-5xl xl:text-6xl">
        {product.name}
      </h1>
      <p className="mt-2 text-sm text-[#1C1412]/55">{product.nameFr}</p>

      <div className="mt-5 flex items-end gap-2">
        <p className="text-4xl font-semibold leading-none text-[#C45B6A] md:text-[2.35rem]">
          {product.price1}
      ...
      <ul className="mt-5 grid grid-cols-1 gap-2 sm:grid-cols-2">
        {BUY_TRUST.map((item) => (
          <li key={item.t} className="flex gap-2.5 border border-[#1C1412]/8 bg-[#F7F1EC]/80 px-3 py-2.5">
            <span className="mt-0.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[#C45B6A]" />
            <span>
              <span className="block text-[12px] font-semibold text-[#1C1412]">{item.t}</span>
              <span className="block text-[11px] text-[#1C1412]/45">{item.d}</span>
            </span>
          </li>
        ))}
      </ul>
      ...
      <ul className="mt-5 space-y-2">
        {product.features.slice(0, 4).map((line) => (
          <li key={line} className="flex gap-2.5 text-[13px] leading-6 text-[#1C1412]/80">
            <span className="mt-[7px] h-1.5 w-1.5 shrink-0 rounded-full bg-[#C4A484]" />
            {line}
          </li>
        ))}
      </ul>
      ...
      <div className="mt-6 space-y-2">
        <button type="button" onClick={() => onAdd(product.price1, 1)} className="btn btn-primary btn-block btn-lg min-h-14">
          <span className="sm:hidden">Acheter — {product.price1} Dhs</span>
          <span className="hidden sm:inline">Acheter maintenant — {product.price1} Dhs</span>
        </button>
        ...
        <p className="text-center text-[12px] font-medium text-[#1C1412]/50">Paiement à la livraison</p>
      </div>
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-white pb-28 md:pb-0">
      ...
      <section className="bg-white">
        <div className="container mx-auto grid items-start gap-5 px-4 py-4 md:grid-cols-2 md:gap-8 lg:gap-14 lg:px-8 lg:py-10 xl:grid-cols-[minmax(0,1fr)_400px]">
          <div className="min-w-0">
            <div className="relative h-[min(52svh,420px)] overflow-hidden bg-[#F7F1EC] md:h-[min(70vh,640px)]">
            ...
            <div className="-mx-4 mt-3 flex gap-2 overflow-x-auto px-4 pb-1 md:mx-0 md:flex-wrap md:overflow-visible md:px-0">
              {product.gallery.map((img, i) => (
                <button
                  ...
                  className={`h-14 w-14 shrink-0 overflow-hidden border md:h-[68px] md:w-[68px] ${
      ...
      <section className="bg-[#F7F1EC] py-10 md:py-16">
        <div className={`container mx-auto grid items-start gap-10 px-4 ${beforeAfter ? "lg:grid-cols-2 lg:gap-16" : ""}`}>
          <div>
            <p className="text-[11px] font-medium tracking-[0.28em] text-[#C4A484]">RAONAQ {product.name}</p>
            <h2 className="font-display mt-2 max-w-xl text-3xl font-semibold text-[#1C1412] md:text-4xl">
              {product.techTitle}
            </h2>
            <ul className="mt-6 space-y-3 md:mt-8">
              {product.techPoints.map((point) => (
                <li key={point} className="flex gap-3 text-[15px] leading-7 text-[#1C1412]/75">
                  <span className="mt-2.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[#C45B6A]" />
                  {point}
                </li>
              ))}
            </ul>
          ...
          {beforeAfter && (
            <div>
              ...
              <h2 className="font-display mt-2 text-3xl font-semibold text-[#1C1412] md:text-4xl">La différence se voit</h2>
      ...
      <section className="border-y border-[#1C1412]/8 bg-white">
        <div className="container mx-auto grid gap-px sm:grid-cols-2 lg:grid-cols-4">
          {PDP_PROOF.map((item) => (
            <div key={item.t} className="px-5 py-6">
              <p className="text-[13px] font-semibold text-[#1C1412]">{item.t}</p>
              <p className="mt-1 text-[12px] leading-5 text-[#1C1412]/50">{item.d}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-[#F7F1EC] py-10 md:py-16">
        <div className="container mx-auto px-4">
          ...
          <p className="mt-3 max-w-xl text-sm leading-6 text-[#1C1412]/55">
            {product.protectHow} En cas de doute, commencez toujours plus bas.
          </p>
          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {product.hairGuide.map((item) => (
              <article key={item.label} className="bg-white px-5 py-5">
                ...
                <p className="mt-2 text-sm font-semibold text-[#C45B6A]">{item.setting}</p>
                <p className="mt-1.5 text-[13px] leading-6 text-[#1C1412]/55">{item.note}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white py-10 md:py-16">
        <div className="container mx-auto px-4">
          ...
          <h2 className="font-display mt-2 text-3xl font-semibold text-[#1C1412] md:text-4xl">En quatre étapes</h2>
          <p className="mt-2 text-sm text-[#1C1412]/45">{product.styleTime}</p>
          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {product.howTo.slice(0, 4).map((step, i) => (
              <article key={step} className="overflow-hidden bg-[#F7F1EC]">
      ...
      <section className="bg-white py-10 md:py-16">
        <div className="container mx-auto max-w-3xl px-4">
          <p className="text-[11px] font-medium tracking-[0.28em] text-[#C4A484]">AVANT DE COMMANDER</p>
          <h2 className="font-display mt-2 text-3xl font-semibold text-[#1C1412]">Questions</h2>
