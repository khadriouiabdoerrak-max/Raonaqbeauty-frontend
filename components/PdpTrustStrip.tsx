...
function TrustChip({
  ...
  item: (typeof PDP_TRUST_STRIP)[number];
}) {
  return (
...
/** Bandeau confiance animé (marquee) — style Massima, promesse Raonaq */
export default function PdpTrustStrip() {
  const loop = [...PDP_TRUST_STRIP, ...PDP_TRUST_STRIP, ...PDP_TRUST_STRIP];

  return (
    <section className="border-y border-[#1C1412]/8 bg-white" aria-label="Engagements Raonaq">
      <div className="overflow-hidden py-5">
        <div className="raonaq-trust-track flex w-max items-center">
          {loop.map((item, i) => (
